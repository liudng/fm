...
void ShortcutManager::detectConflicts() {
    ...
    for (auto it = items_.begin(); it != items_.end(); ++it) {
        const QString key = it.value().currentKey.isEmpty()
                                ? it.value().defaultKey : it.value().currentKey;
...
void ShortcutManager::loadFromConfig() {
    ...
    for (auto it = items_.begin(); it != items_.end(); ++it) {
        const QString key = cfg->value(QStringLiteral("Shortcuts"), it.key(),
                                        it.value().defaultKey).toString();
