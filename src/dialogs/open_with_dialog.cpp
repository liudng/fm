    ...
    for (const QString &mapPath : mimeappsPaths) {
        ...
        while (ts.readLineInto(&line)) {
            ...
            const QStringList entries =
                line.mid(eq + 1).split(QLatin1Char(';'), Qt::SkipEmptyParts);
    ...
    : QDialog(parent), mimeType_(mimeType)
{
    ...
    connect(appList_, &QListWidget::itemSelectionChanged, this,
            &OpenWithDialog::onSelectionChanged);
}
...
{
    const QString path =
        QFileDialog::getOpenFileName(this, tr("Select Application"), QStringLiteral("/usr/bin"));
    if (!path.isEmpty()) {
