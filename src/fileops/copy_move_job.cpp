...
int copyFileChunked(const QString &src, const QString &dst, qint64 *processedBytes,
                    qint64 totalBytes, FileJob *job, const QString &fileName)
{
    QFile srcFile(src);
    ...
    const qint64 chunkSize = static_cast<qint64>(ConfigManager::instance()
                                                     ->value(QStringLiteral("File_Operations"),
                                                             QStringLiteral("chunkSizeMB"), 1)
                                                     .toInt()) *
                             1024 * 1024;
    QByteArray buffer;
    ...
    while (!srcFile.atEnd()) {
        ...
        *processedBytes += n;
        const int percent =
            (totalBytes > 0) ? static_cast<int>((*processedBytes * 100) / totalBytes) : 0;
...
// 递归复制目录（带进度报告）
// 返回值：0=成功, 1=用户取消, -1=失败
int copyRecursively(const QString &src, const QString &dst, QString *error, qint64 *processedBytes,
                    qint64 totalBytes, FileJob *job)
{
    ...
    if (srcInfo.isFile()) {
        ...
        if (ret != 0) {
            if (ret == -1 && error)
                *error = QCoreApplication::translate("fm::CopyMoveJob", "Cannot copy: %1 -> %2")
                             .arg(src, dst);
            return ret;
        }
        return 0;
    ...
    QDir srcDir(src);
    const QStringList entries =
        srcDir.entryList(QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot | QDir::Hidden);
    for (const QString &e : entries) {
...
CopyMoveJob::CopyMoveJob(const QList<QUrl> &sources, const QString &destDir, bool isMove,
                         QObject *parent)
    : FileJob(parent), sources_(sources), destDir_(destDir), isMove_(isMove), resolver_(this)
{}
    ...
    for (const QUrl &u : sources_) {
        ...
        if (QFileInfo(src).isDir()) {
            ...
            if (!srcCanon.isEmpty() && !dstCanon.isEmpty() &&
                (dstCanon == srcCanon || dstCanon.startsWith(srcCanon + QLatin1Char('/')))) {
                ErrorDialog::show(nullptr, tr("Cannot copy folder \"%1\" into itself.").arg(name));
                continue;
            }
        }
        ...
        if (QFileInfo::exists(dst)) {
            ...
            if (r == ConflictResolution::Skip || r == ConflictResolution::SkipAll) continue;
            if (r == ConflictResolution::Rename || r == ConflictResolution::RenameAll) {
                QString newName = uniqueName(destDir_, name);
                dst = destDir_ + QDir::separator() + newName;
            }
            // Overwrite / OverwriteAll: 删除目标
            if (r == ConflictResolution::Overwrite || r == ConflictResolution::OverwriteAll) {
    ...
    for (const auto &p : plan_) {
        ...
        if (isMove_) {
            if (!QFile::rename(p.first, p.second)) {
                // 跨设备，复制+删除
                const int ret =
                    copyRecursively(p.first, p.second, error, &processedBytes, totalBytes_, this);
